...
usage() {
  cat <<'EOF'
Install the prtags CLI from GitHub Releases.

Usage:
  install-prtags.sh [--version vX.Y.Z] [--bin-dir DIR]
...
Environment:
  PRTAGS_VERSION      Same as --version.
  PRTAGS_INSTALL_DIR  Same as --bin-dir.
EOF
}
...
VERSION="${PRTAGS_VERSION:-}"
INSTALL_DIR="${PRTAGS_INSTALL_DIR:-}"
